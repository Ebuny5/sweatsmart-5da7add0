/**
 * HidroAlly, HidroAlly Recommendation Engine (Definitive Edition)
 * ─────────────────────────────────────────────────────────────────────────────
...
function areaPhrase(areas: string[]): string {
  const mappedAreas = areas.map((a: string) => {
    const areaStr = a.toLowerCase().trim();
    if (areaStr.includes('underarm')) return 'Axillary vaults (underarms)';
  ...
  if (mappedAreas.length === 2) return `${mappedAreas[0]} and ${mappedAreas[1]}`;
  const formattedAreas = `${mappedAreas.slice(0,-1).join(", ")}, and ${mappedAreas[mappedAreas.length-1]}`;
  return `Multifocal primary focal hyperhidrosis (${formattedAreas})`;
}
...
function buildClinical(
  ...
  const isNoTrigger = has(triggers, "no trigger", "no identifiable trigger", "spontaneous", "idiopathic", "none");
  ...
  const mechText = mechs.length > 0
    ? pick([
        ...
        ` The underlying sweat pathway is hyperreactive, it amplifies normal signals into disproportionate responses. Here, that hyperreactivity was fed by: ${mechs.slice(0,4).join("; ")}.`,
        ` The sweat glands in these areas overreact to signals that most bodies would barely register. What drove this episode: ${mechs.slice(0,4).join("; ")}.`,
      ], seed+2)
  ...
  if (p.isPossibleSecondary) {
    ...
    close = pick([
      " Since there was no identifiable trigger, this is a classic presentation of spontaneous autonomic sympathetic discharge. Tracking these unprovoked episodes is just as important as tracking triggered ones, as it provides a clear baseline of your neurological threshold.",
      " Spontaneous episodes without a clear cause are a hallmark of primary focal hyperhidrosis. This confirms that your sweat glands are firing based on internal autonomic rhythms rather than external catalysts.",
      " Do not worry about searching for a hidden trigger. Primary hyperhidrosis often operates independently of external factors. Focus your tracking on the frequency and severity of these spontaneous events."
    ], seed+4);
  } else {
    close = pick([
...
function buildTreatments(
  ...
  if ((hasPalmoplantar || hasAxillary) && options.length < 2) {
    // We remove the redundant facial warning from this section to obey the rule:
    // "Do not mention the facial warning inside the underarm section if a dedicated craniofacial section follows."
    const facialWarning = "";
...
export function generateEpisodeInsights(input: EpisodeInput): EpisodeInsights & { cta: string; emotionalOpener: string } {
  ...
  // Dry days have no symptoms to analyse, completely bypass the clinical
  // pattern engine and return a treatment-compliance / management-focused
...
// ─── Dry day response ──────────────────────────────────────────────────────────
// Bypasses clinical/symptom analysis entirely. Rotates between 5 supportive,
// treatment-compliance-focused themes per spec. Never mentions body areas,
// triggers, or "missing data", a dry day is a successful outcome, not an
// incomplete log.
