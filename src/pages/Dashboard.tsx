            ...
            <div className="bg-white rounded-2xl shadow-sm border border-purple-100 overflow-hidden">
              ...
              <div className="p-4 space-y-2">
                {dashboardData.bodyAreas.slice(0, 5).map((area, index) => {
                  ...
                  const label = area.area.replace(/_/g, ' ').replace(/\w/g, c => c.toUpperCase());
                  const colors = [
                    'bg-violet-500', 'bg-purple-400', 'bg-pink-400', 'bg-indigo-400', 'bg-fuchsia-400'
                  ];
                  return (
                    <div key={area.area}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-semibold text-gray-700">{label}</span>
                        <span className="text-xs text-gray-400">{area.count} episodes</span>
                      </div>
                      <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${colors[index] || 'bg-violet-400'}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
